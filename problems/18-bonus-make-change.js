...
function makeBetterChange(target, coins = [25, 10, 5, 1]) {
  ...
  for (let i = 0; i < coins.length; i++) {
      ...
      // Skip if coin is too big
      if (coin > target) continue;

      let remainder = target - coin;
      // Recurse with remainder, using only coins less than or equal to current coin
      let bestRemainder = makeBetterChange(remainder, coins.slice(i));

      // Skip if no change can be made for remainder
      if (!bestRemainder) continue;

      let change = [coin, ...bestRemainder];

      // Choose better change if it's shorter (fewer coins)
